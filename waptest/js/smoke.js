...
var canvasSmoke = function(canvas, options) {
	...
	function Particle(context) {
		...
		this.update = function() {
			...
			// 越靠近边缘，透明度越低
			// 纵向透明度变化要比横向的明显
			this.alpha = (1 - Math.abs(canvasWidth *0.1 - this.x) / canvasWidth) * ( 1 - Math.abs(canvasHeight *0.1 - this.y) / canvasHeight);
		};
...
// IE9+烟雾效果走起
if ([].map) {
	canvasSmoke(document.querySelector('#smokeCanvas1'));
	canvasSmoke(document.querySelector('#smokeCanvas2'));
}
